...
//			StringBuffer sb = new StringBuffer();
	...
	private Dimension getPersonSize(Graphics g) {
		...
		if (occuw > namew) namew = occuw;
		imagew += separatorWidth;
		if (birtw > namew) namew = birtw;
		if (deatw > namew) namew = deatw;
	...
	private Dimension getTableSize(Graphics g) {
		...
		if (getChildCount() > 0){
			...
			if (childic>0){
				...
				if (childic > imagew) imagew = childic;
			}
	...
	public PersonShortData getSubject(){
		if (famMember.size() == 0) return null;
