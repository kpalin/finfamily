...
/**
 * Admin part of application uses database directly from user side
 * This is not available in webstart version
 * 
 * @author Kalle
 *
 */
...
//		JMenuItem mConn = new JMenuItem(Resurses.getString(Resurses.CONNECT));
//		mFile.add(mConn);
...
//		setVisible(true);
	...
	private Connection getConnection() throws SQLException{
		if (this.postPassword == null) return null;
	...
	public boolean connectPostgres(){
		...
		//this.postPassword = JOptionPane.showInputDialog(Resurses.getString(Resurses.CONNEADMIN));
		String [] datalista;
		String [] userlista;
		try {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (cmd.equals(Resurses.NEWUSER)){
			...
			if (newUser != null && newPassword != null && newUser.length() > 2 && newPassword.length() > 2){
				...
				try {
					String sql = "CREATE ROLE " + newUser + " WITH LOGIN PASSWORD '"+newPassword+"' ";

					Connection con = getConnection();
					Statement stm = con.createStatement();
					stm.executeUpdate(sql);
					
					String []userlista = LocalDatabaseUtility.getListOfUsers(con);
					this.userCombo.removeAllItems();
					for (int i = 0; i < userlista.length; i++) {
						this.userCombo.addItem(userlista[i]);
			...
			if (newPassword != null ){
				try {
					String sql = "alter user " + oldUser + " password  '" + newPassword + "'";
				 	Connection con = getConnection();
					Statement stm = con.createStatement();
					stm.executeUpdate(sql);
					con.close();
			...
			String newDb = JOptionPane.showInputDialog(this, Resurses.getString("NEWDATABASENAME"));
			...
			if (owner != null && newDb != null && newDb.length() > 0 ){
				try {
				    String sql = "create database " + newDb + " owner = " + owner + " encoding = 'UTF8'";
			        Connection con = getConnection();
					Statement stm = con.createStatement();
					stm.executeUpdate(sql);
					String[]datalista = LocalDatabaseUtility.getListOfDatabases(con);
					this.dblista.setListData(datalista);
			...
			if (JOptionPane.showConfirmDialog(this, Resurses.getString("ASKTODROP") + " " + dbname)== JOptionPane.YES_OPTION) {
