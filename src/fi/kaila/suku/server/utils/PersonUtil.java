		...
		try {
			...
			for (int i = 0; i < nn.length; i++) {
				...
				if (n.isToBeDeleted()){
					...
					String text = "Poistettiin " + delcnt + " riviä [" + landelcnt + "] kieliversiota pid = " 
					...
					if (n.getPnid()>0){
						...
						//						System.out.println("Päivitettiin " + luku + " tietuetta");
						...
						int luku = pst.executeUpdate();
						//						System.out.println("Luotiin " + luku + " uusi tietue");
					...
					if (n.getMediaData() == null){
						...
						int lukuri= pst.executeUpdate();
						if (lukuri != 1){
							logger.warning("media deleted for pnid " +n.getPnid() + 
									" gave result " + lukuri );
						} 
					} else {
					...
					for (int l = 0; l < n.getLanguages().length; l++) {
						...
						if (ll.isToBeUpdated()){
							...
							if (ll.getPnid()==0){
								...
								int lukuri= pst.executeUpdate();
								if (lukuri != 1){
									logger.warning("language for pnid " +ll.getPnid() + 
											" [" + ll.getLangCode() + "] gave result " + lukuri );
								}
								pst.close();
							}
		...
		try {
			...
			for (int i = 0; i < req.relations.length; i++) {
				...
				if (req.relations[i].isToBeDeleted() && rid > 0) {
					...
					logger.info("deleted relation [" + r.getTag() + "" + rid + " between "
							+ r.getPid() + "/" + r.getRelative() + " result [" + laskRel + "/" + laskNoti + "/" + laskLang);
					...
					if (rid == 0) {
						...
						if (r.getPid() == req.persLong.getPid()){
							...
							if (r.getTag().equals("FATH") || r.getTag().equals("MOTH")){
								...
								int lukuri= pst.executeUpdate();
								if (lukuri != 1){
									logger.warning("other relation for rid " +rid + " [" + r.getTag() +
											"]  gave result " + lukuri );
								}
				...
				if (req.relations[i].getNotices() != null) {
					...
					for (int j = 0; j < req.relations[i].getNotices().length; j++) {
						...
						if (rn.isToBeDeleted() && rnid>0){
							...
							logger.info("deleted relationnotice [" + r.getTag() + "" + rid + " between "
									+ r.getPid() + "/" + r.getRelative() + " result [" + laskNoti + "/" + laskLang);
						} else {
							if (rn.isToBeUpdated() || rnid==0){

								if (rn.getRnid()==0){
									stm = con.createStatement();
									ResultSet rs = stm.executeQuery("select nextval('RelationNoticeSeq')");
								...
								if (rn.getRnid()>0){
									...
									System.out.println("update rn for " + rnid + "[" + rer + "]");
								} else {
									pst.setInt(10, rid);
									pst.setString(11, rn.getTag());
									int rer = pst.executeUpdate();
									System.out.println("insert rn for " + rnid + "[" + rer + "]");
							...
							if (rn.getLanguages() != null){
								...
								for (int k = 0; k < rn.getLanguages().length; k++){
									...
									if (rl.isToBeUpdated()) {
										if (rl.getRnid() == 0){
											PreparedStatement updLang = con.prepareStatement(insLangSql);
											//										"(rnid,rid,langcode,RelationType,Description,Place,NoteText) " +
											...
											System.out.println("insert rl rnid: " + rnid + "/"+rl.getLangCode()+ "count:[" + rier + "]");



										} else if (rl.isToBeDeleted()) {
											PreparedStatement updLang = con.prepareStatement(delLangSql);
											updLang.setInt(1, rnid);									
											updLang.setString(2,rl.getLangCode());
											int rder = updLang.executeUpdate();
											System.out.println("delete rl rnid: " + rnid + "/"+rl.getLangCode()+ "count:[" + rder + "]");

										} else {

											PreparedStatement updLang = con.prepareStatement(updLangSql);
											...
											System.out.println("update rl for " + rl.getRnid() + "/"+rl.getLangCode()+ "[" + rner + "]");
										}
									}
								}
							}
			...
			for (int i = 0; i < req.relations.length; i++) {
				...
				if (r.getPid() == req.persLong.getPid()){
					...
					int lukuri = pst.executeUpdate();
					if (false) System.out.println("RELAROW # " + lukuri);
