		...
		try {
			...
			for (int i = 0; i < notices.length; i++) {
				...
				if (notice.getMediaFilename()!= null){
					...
					//append("\n\n" + notice.getMediaFilename() + " (" + notice.getMediaSize().width + "," + notice.getMediaSize().height + ")\n",bodyText);
					append("\n\n          ",bodyText);
...
//								newh = imh; 
				...
				bl = appendName(bodyBold,underlinedBold,bl,notice.getGivenname(),
						notice.getPatronym(),notice.getPrefix(),notice.getSurname(),notice.getPostfix());
			...
			if (relations != null && relations.length > 0 && namlist != null){
				...
				for (int i = 0; i < relations.length; i++){
					...
					if (rel.getTag().equals("FATH")|| rel.getTag().equals("MOTH")){
						...
						bl = appendName(bodyBold,underlinedBold,bl,relative.getGivenname(),
								relative.getPatronym(),relative.getPrefix(),relative.getSurname(),relative.getPostfix());
...
//						append(bl + relative.getAlfaName(),bodyText);
			...
			//sb.append(Resurses.getString("TEXT_PID") + " = " + pers.getPid()+"\n" );
