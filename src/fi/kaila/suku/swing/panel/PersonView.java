...
public class PersonView extends JPanel implements ChangeListener{
	 ...
	 * @param suku = pointer to main program
...
//			int fnotice = getFirstNoticeIndex();
		...
		getSuku().setTitle(ps.getAlfaName() + " " 
				+ nv4(ps.getBirtDate()) + "-" + nv4(ps.getDeatDate()));
	 ...
	 * 
	 * @param reOpen true if page to be reopened 
	 * @throws SukuException 
	 */
...
//		System.out.println("Talletetaan henkilö: " +personPid);
	...
	public void setSubjectForFamily(PersonShortData subject) throws SukuException{
		
		
		SukuData family = Suku.kontroller.getSukuData("cmd=family","pid=" + subject.getPid(),"parents=both");
		...
		if (family.pers == null || family.pers.length == 0) {
			...
			table.setSubject(family.getSubject());
			...
			for (int i = 0; i < fcount+mcount; i++) {
				...
				ftab.setSubject(family.getSubject());
				...
				famPanel.addRels(new FamilyParentRelationIndex(0,parents.size()));
				
				
				Dimension dd = ftab.getSize(gg);
				
				x += dd.width + 30;
				if (reqDim.width < x)reqDim.width = x;
				if (maxheight < dd.height) maxheight = dd.height;
				pareWidth = x;
			}
			x = 0;
			y = 10;
...
//				ftab.initRelatives(family.pers, family.rels);
				ftab.setLocation(new Point(x,y));
				famPanel.addTable(ftab);
				famPanel.addRels(new FamilyParentRelationIndex(rel.getAux(),famPanel.getTabSize()-1));
				
				Dimension dd = ftab.getSize(gg);
				
				x += dd.width + 30;
				if (gheight < dd.height) gheight = dd.height;
			...
			table.setLocation(new Point(rw/2-xw/2,maxheight + gheight + y + 50));
...
//		this.famPanel.setPreferredSize(new Dimension(1200, 1600));
	...
	public void setTextForPerson(PersonShortData person) throws SukuException {
		...
		SukuData pdata = Suku.kontroller.getSukuData("cmd=person","pid=" + person.getPid(),"lang="+Resurses.getLanguage());
		SukuData family = Suku.kontroller.getSukuData("cmd=family","pid=" + person.getPid(),"parents=yes");
		PersonTextPane textPerson = (PersonTextPane)paneTabs.get(1).pnl;
	...
	public StyledDocument getDoc() {
		PersonTextPane textPerson = (PersonTextPane)paneTabs.get(1).pnl;
		if (textPerson == null)return null;
...
;
	...
	public void stateChanged(ChangeEvent e) {
		...
		int mnotice = getMainPaneIndex();
		if (main != null ) {
			if (previousNoticeIndex >= fnotice && previousNoticeIndex< getTabCount()) { // greater than sukulaiset
				NoticePane pane = null;
				...
				if (resu != null) {
					...
					skipNextState=true;
					//TODO tarkista viel�k� t�m� on tarpeen. N�ytt�isi olevan 14.9.09
					tabbedPane.setSelectedIndex(previousNoticeIndex);
