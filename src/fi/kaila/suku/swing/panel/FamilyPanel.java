...
//			System.out.println("rel:" + rel.getChildIdx() + "/" + rel.getParentIdx());
		...
		//	cp.x += dd.width/2;
			...
			g.drawLine(cp.x + dd.width/2, cp.y, pp.x+dp.width/2, pp.y+dp.height);
...
//		System.out.println("addrel: " + relIdx.getChildIdx() + "/" + relIdx.getParentIdx());
			...
			for (int i = 0; i < tabs.size(); i++) {
				...
				if (dd != null){
					if (dd.contains(pp)){
						...
						PersonShortData person = subjectTable.getPersonAtPoint(point);
						if (person != null) {
							try {
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (e.getButton()== MouseEvent.BUTTON3 && e.getClickCount()==1){
			...
			for (int i = 0; i < tabs.size(); i++) {
				...
				if (dd.contains(pp)){
//					System.out.println("Osui: ");// + subjectTable.getSubject().getTextName());
					Point point = new Point(pp.x-dd.x,pp.y-dd.y); 
...
//						System.out.println("Henkilöön: " + person.getTextName());
					} else {
