...
//		}
		...
		//JOptionPane.showMessageDialog(this, "Paikkoja [" + paikkoja + "]. Käynti no " + lukuri);
	}
	...
	class KarttaPanel extends JPanel implements MouseListener,MouseMotionListener{
		...
		public void paintComponent(Graphics g) {
			...
			//Rectangle suomiSize = new Rectangle(0,0,kartta.getWidth(),kartta.getHeight());
	...
	//            gg.drawString("KALLEN KOE", 19, 59);
			...
			for (idx = 0; idx < places.length; idx++) {
				...
				if (place.getLatitude() > 0){
					 ...
					 Rectangle rec = convertToRectangle(place.getLongitude(), place.getLatitude(), place.getCount());

//                     if (debWrite)
//                     {
//                         Debug.WriteLine(place.ToString() + " to " + rec.ToString());
//                     }
...
//			PointD xyz4 = convertToPointD(33.4, 70.3);  // left bottom
		    ...
		    if (count > 20) displayCount = 20;
		    return new Rectangle((int)(pp.x - displayCount * 2 / 2), (int)(pp.y - displayCount * 2 / 2), displayCount * 2, displayCount * 2);
		}
            ...
            if (mouseIsHere){
            	...
            	for (idx = 0; idx < places.length; idx++) {
    				...
    				if (place.getLatitude() > 0){
    					...
    					if (rec.contains(x, y)){
    						String nytPaikka = place.getName() + "(" + place.getCount() + ")";
