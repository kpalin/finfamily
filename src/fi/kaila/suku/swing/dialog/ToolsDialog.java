...
public class ToolsDialog extends JDialog implements ActionListener,
	...
	public ToolsDialog (Suku owner){
		...
		try {
			reposet = Suku.kontroller.getSukuData("cmd=gettypes","lang="+Resurses.getLanguage());
		...
		textContent = new JLabel("Tämä käy läpi kaikki tietokantaikkunan henkilöt");
		...
		if (cmd.equals(OK)){
			...
			request.generalArray=settiTags.toArray(new String[0]);
		  	...
		  	try {
				...
				JOptionPane.showMessageDialog(this, ee.getMessage(), Resurses.getString(Resurses.SUKU), JOptionPane.ERROR_MESSAGE);
				ee.printStackTrace();
			}
