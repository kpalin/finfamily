 ...
 * only one group and he/she can belong to several views. Group Manager manages the groups.</p>
 * 
 * <p>The Group manager consists of a non-modal Dialog that is requested
 * from the Tools menu.</p>
 * 
 * <p>GroupId in Unit table can contain any string or be null. Updating of GroupId is
 * made with requests from here. Update possibility for GroupId from PersonView has been
 * removed to avoid updateing from two sources simultaneuosly.</p>
 * 
 * <h2>Remove Group</h2>
 * 
 * <p>This group of commands can remove groupId from all persons, 
 * from persons selected in database view or from persons found
 * from selected view.</p>
 * 
 * <h2>Add to Group</h2>
 * 
	...
	private void updateSelectStatus(){
		try {
			...
			if (pp == null) {
				...
				if (pids==null) return;
				selectedName.setText(Resurses.getString("DIALOG_GROUP_SELECTED_COUNT") + 
						" " + pids.length);
			} else {
			...
			try {
				...
				if (model != null) {
					...
					if (remocmd.equals("ALL")){
						...
						response = Suku.kontroller.getSukuData(request, "cmd=group",
								"action=remove","view="+viewids[isele]);
					...
					if (response == null || response.pidArray == null) {
						...
						String messu = Resurses.getString("DIALOG_GROUP_REMOVED") +
