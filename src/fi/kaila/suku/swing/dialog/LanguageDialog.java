...
//	public String [] texts = null;
	...
	public UnitLanguage[] getLanguages(){
		...
		for (int i = 0; i < languages.length; i++) {
			...
			if (u.getNoticeType() == null && 
					u.getMediaTitle() == null && 
		...
		if (languages != null) {
			for (int i = 0; i < languages.length; i++) {
				...
				if (idx >= 0) {
					...
					logger.warning("language code not known [" + languages[i].getLangCode()+"]");
				}
			}
...
//		ry += 22;
		...
		scrollArea.setBounds(100,ynote,leftWidth,currSize.height-ynote-40);
		scrollArea.updateUI();
