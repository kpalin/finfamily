 ...
 * 
 * <p>Persons in the database can be grouped in groups and in views. A person can have 
 * only one group and he/she can belong to several views. View Manager manages views.</p>
 * 
 * <p>The View manager consists of a non-modal Dialog that is requested
 * from the Tools menu.</p>
 * 
 * <p>Views are held in two database tabels, the Views table that contains a list 
 * of the named views and their corresponding numeric ViewId's and table
 * viewunits with a list of persons (units) for each view. A person is considered 
 * to be a member of a view if he/she exists (at least once) in the viewunits table.
 * The person can exist in more than one view.</p>
 * 
 * <h2>Create a View</h2>
 * 
 * <p>Write the name of the view to be created in the textfield and press cerate. 
 * If a view with the name already exists an error is thrown.</p>
 * 
 * <h2>Remove View</h2>
 * 
...
//		formd=new JRadioButton(Resurses.getString("DIALOG_VIEW_ADD_SELECTED_EMPTY"));
	...
	private void updateSelectStatus(){
		try {
			...
			if (pp == null) {
				...
				if (pids==null) return;
				selectedName.setText(Resurses.getString("DIALOG_VIEW_SELECTED_COUNT") + 
						" " + pids.length);
			} else {
...
//		System.out.println("CLOSE GROUP FOR " + e.toString());
		if (e.getSource()==close){
			...
			try {
				...
				if (newViewName.equals("")) return ;
				for (int i = 0; i < viewnames.length; i++) {
					if (viewname.equals(viewnames[i])){
						JOptionPane.showMessageDialog(parent, Resurses.getString("DIALOG_VIEW_EXISTS")+" ["+viewname+"]"	);
						return;
					}
				}
					...
					if (remocmd.equals("ALL")){
						...
						if (request.pidArray != null && request.pidArray.length>0){
							response = Suku.kontroller.getSukuData(request, "cmd=view",
									"action=remove","key=pidarray","viewid="+seleview);	
						}
					} else {
