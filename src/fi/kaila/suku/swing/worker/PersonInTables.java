...
public class PersonInTables {
	 ...
	 * @param table
	 * @param alsoOwner is true to list also references as owner
	 * @param alsoChild is true to list also references as child
	 * @param alsoSpouse is true to list also references as spouse
	 * @return a comma seperated list of other table numbers
	 */
	public String getReferences(long table,boolean alsoOwner,boolean alsoChild,boolean alsoSpouse){
		StringBuffer sx = new StringBuffer();
		
		if (alsoChild) {
			for (int i=0; i < asChildren.size(); i++){
				if (asChildren.get(i) != table){
					if (sx.length() > 0) sx.append(",");
