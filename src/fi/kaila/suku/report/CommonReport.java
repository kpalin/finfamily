...
import fi.kaila.suku.report.style.*;
 ...
 * 
 * <p>Reports architecture consist of</p>
 * 
 * <ul>
...
public class CommonReport {
	 ...
	 * 
	 * tab sisältää henkilön ReportTableMember[0]
	 * puolisoiden ReportTableMember[1...n]
	 * lasten ReportTableMember[0..n-1]
	 * 
	 * persLong sisältää FullPerson
	 * katso fi.kaila.suku.server.SukuServerImpl.getFullPerson
	 * 
	 * personReferences sisältää kaikkien henkilöiden esiintymiset taulurakenteessa
	 * 
	 * @param idx
	 * @param tab
	 */
	protected void createTable(int idx,ReportUnit tab) {
		...
		for (int i = 0; i < xnotices.length; i++) {
			if (xnotices[i].getTag().equals("TABLE")) tableCount++;
			if (xnotices[i].getTag().equals("FAMT"))  famtCount++;
		}
		...
		for (int j = 0; j < notices.length; j++){
			...
			if (nn.getTag().equals("NAME")){
				...
				if (tabOwner.length()>0) tabOwner.append(" ");
				tabOwner.append(nn.getGivenname());
				break;
			}
		}
		float prose = (idx * 100f) / tables.size();
		caller.setRunnerValue(""+(int)prose+";"+tab.getTableNo()+":"+ tabOwner);
		...
		if (fromTable.length()>0) {
			bt.addText(caller.getTypeText("ALSO")+" " +fromTable + ". ",true,false );
		}
...
//							System.out.println("REL:" + spouseMember.getPid() + "/" + sdata.relations[i]);
							if (sdata.relations[i].getNotices() != null){
								...
								for (int j = 0; j < rnn.length; j++) {
									...
									spouType = printRelationNotice(rn,spouType,spouNum);
									
									
									
//									System.out.println("RELN:" + sdata.relations[i].getNotices()[j]);
								}
							}
				...
				if (ref != null) {
					typesColumn = ref.getTypesColumn(tab.getTableNo(),  true,true,false);
				}	
				...
				if (ref != null) {
					fromTable = ref.getReferences(tab.getTableNo(),  true,true,false);
					if (fromTable.length()>0) {
						bt.addText(caller.getTextValue("ALSO")+" " +fromTable + ". ",true,false );
					}
				}	
				...
				if (spouseMember.getSubCount() > 0){
					...
					for (int i = 0; i < spouseMember.getSubCount(); i++){
						...
						SukuData sub = caller.getKontroller().getSukuData("cmd=person","pid=" + spouseMember.getSubPid(i));
						...
						if (ref != null) {
							fromTable = ref.getReferences(tab.getTableNo(),  true,true,true);
							if (fromTable.length()>0) {
								bt.addText(caller.getTextValue("ALSO")+" " +fromTable + ". ",true,false );
							}
						}
			...
			try {
				...
				for (int i = 0; i < cdata.relations.length; i++){
					if (ownerTag.equals(cdata.relations[i].getTag())){
						if (cdata.relations[i].getNotices() != null){
							adopTag=cdata.relations[i].getNotices()[0].getTag();
				...
				if (childMember.getSubCount() > 0){
					...
					for (int i = 0; i < childMember.getSubCount(); i++){
						...
						SukuData sub = caller.getKontroller().getSukuData("cmd=person","pid=" + childMember.getSubPid(i));
						notices = sub.persLong.getNotices();
						printName(bt, notices,4);
						printNotices(bt, notices, 4);
				...
				if (!fromTable.equals("")){
					bt.addText(caller.getTextValue("TABLE")+" " + fromTable+". ",true,false);
					repoWriter.addText(bt);
					...
					if (childMember.getSpouses() != null && childMember.getSpouses().length > 0){
						...
						for (int j = 0; j < childMember.getSpouses().length; j++) {
							...
							SukuData child = caller.getKontroller().getSukuData("cmd=person","pid=" + childSpouseMember.getPid());
							String tmp;
							...
							if (child.relations != null) {
								...
								for (int i = 0; i < child.relations.length; i++){
									if (child.relations[i].getRelative() == tab.getPid()){
							...
							fromTable="";
						//	ref = personReferences.get(child.persLong.getPid()) ;
							if (ref != null) {
								fromTable = ref.getReferences(tab.getTableNo(),  true,true,false);
								if (fromTable.length()>0) {
									bt.addText(caller.getTextValue("ALSO")+" " +fromTable + ". ",true,false );
							...
							if (childSpouseMember.getSubCount() > 0){
								...
								for (int i = 0; i < childSpouseMember.getSubCount(); i++){
									...
									SukuData sub = caller.getKontroller().getSukuData("cmd=person","pid=" + childSpouseMember.getSubPid(i));
									notices = sub.persLong.getNotices();
									printName(bt, notices,4);
									printNotices(bt, notices, 4);

									fromTable="";
									ref = personReferences.get(childSpouseMember.getSubPid(i)) ;
									if (ref != null) {
										fromTable = ref.getReferences(tab.getTableNo(),  true,true,true);
										if (fromTable.length()>0) {
											bt.addText(caller.getTextValue("ALSO")+" " +fromTable + ". ",true,false );
	...
	private String printRelationNotice( RelationNotice rn,String defType,int spouseNum) {
		...
		String date = printDate(rn.getDatePrefix(),rn.getFromDate(),rn.getToDate());
		if (date.length()>0) {
			if (addSpace) {
				sb.append(" ");
			}
				...
				if (nn.getPrivacy()== null){
					if (( caller.isType(tag, colType ) ) ) {
						...
						if (nn.getDescription() != null) {
							if(addSpace) 	bt.addText(" ");
							bt.addText(nn.getDescription());
							addSpace=true;
							addDot=true;
						}
						
						String dd = printDate(nn.getDatePrefix(),nn.getFromDate(),nn.getToDate());
						if (dd.length()>0){
							if (addSpace) {
								bt.addText(" ");
								bt.addText(dd);
						...
						if (caller.showAddress()){
							...
							if (nn.getAddress() != null) {
								if(addSpace) 	bt.addText(" ");
								int tlen=0;
								String parts[] = nn.getAddress().replaceAll("\\r", "").split("\n");
						...
						if (nn.getMediaFilename()!= null && caller.showImages()){
...
//		}
	...
	/**
	 * print name to report. First name is always printed regardless of types settings
	 * unless set as privacy = true
	 * 
	 * TODO special underline such as  "Anna*-Liisa" (Juha-Pekka*) 
	 * TODO underline Anna-Liisa** or Per Erik** to result in two names underlined
	 * 
	 * @param bt
	 * @param notices
	 * @param isMain
			...
			if (nn.getTag().equals("NAME")){
				if (nn.getPrivacy()== null){
					if (( caller.isType("NAME", colType )|| nameCount==0 ) ) {
							...
							for (int k = 0; k < nameParts.length; k++) {
								...
								if (astidx > 0){
									if (astidx == nameParts[k].length()-1){
										bt.addText(nameParts[k].substring(0,astidx),caller.showBoldNames(),caller.showUnderlineNames());
										
									} else {
										bt.addText(nameParts[k],caller.showBoldNames(),caller.showUnderlineNames());
									}
								} else {
									bt.addText(nameParts[k],caller.showBoldNames(),false);
								}
						...
						if (!prevGivenname.equals("") && !nv(nn.getPrefix()).equals("")){
							bt.addText(" ",caller.showBoldNames(),false);
